    ...
    `}setupEventListeners(){let e=document.querySelector(`#quantity-increase`);e&&e.addEventListener(`click`,()=>{this.quantity++,this.updateQuantityDisplay()});let t=document.querySelector(`#quantity-decrease`);t&&t.addEventListener(`click`,()=>{this.quantity>1&&(this.quantity--,this.updateQuantityDisplay())});let n=document.querySelector(`#quantity-input`);n&&n.addEventListener(`change`,e=>{let t=parseInt(e.target.value);t>=1&&(this.quantity=t),this.updateQuantityDisplay()});let r=document.querySelector(`#add-to-cart-btn`);r&&r.addEventListener(`click`,()=>{let e={id:this.product.productId,name:this.product.title,price:parseInt(this.product.lprice),image:this.product.image,brand:this.product.brand||``};f.addItem(e,this.quantity)});let i=document.querySelectorAll(`.related-product-card`);i.forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.productId;t&&(window.history.pushState({},``,`/product/${t}`),window.dispatchEvent(new Event(`popstate`)))})});let a=document.querySelector(`#breadcrumb-home`);a&&a.addEventListener(`click`,()=>{window.history.pushState({},``,`/`),window.dispatchEvent(new Event(`popstate`))});let o=document.querySelector(`#cart-icon-btn`);o&&o.addEventListener(`click`,()=>{f.openModal()})}updateQuantityDisplay(){let e=document.querySelector(`#quantity-input`);e&&(e.value=this.quantity)}async render(){await this.loadProduct();let e=this.generateHtml();document.querySelector(`#root`).innerHTML=e,!this.loading&&this.product&&(this.setupEventListeners(),f.updateCartIcon())}};const m=`/front_6th_chapter1-1`,h=(e=window.location.pathname)=>e.startsWith(m)?e.slice(21)||`/`:e,g=e=>m+e,_=()=>r(async()=>{let{worker:e,workerOptions:t}=await import(`./browser-D8FlMt87.js`);return{worker:e,workerOptions:t}},[]).then(({worker:e,workerOptions:t})=>e.start(t));let v={products:[],categories:[],total:0,loading:!1,limit:20,sort:`price_asc`,page:1,hasMore:!0,search:``,category1:``,category2:``};function y(){let e=new URLSearchParams(window.location.search);return{search:e.get(`search`)||``,category1:decodeURIComponent(e.get(`category1`)||``),category2:decodeURIComponent(e.get(`category2`)||``),sort:e.get(`sort`)||`price_asc`,limit:parseInt(e.get(`limit`))||20}}function b(){let e=new URLSearchParams;v.search&&e.set(`search`,v.search),v.category1&&e.set(`category1`,v.category1),v.category2&&e.set(`category2`,v.category2),v.sort!==`price_asc`&&e.set(`sort`,v.sort),e.set(`limit`,v.limit.toString());let t=`${window.location.pathname}${e.toString()?`?`+e.toString():``}`;window.history.replaceState({},``,t)}let x=!1,S=null;function C(){let e=h(),t=e.match(/^\/product\/(\d+)$/);return t?{type:`product`,productId:t[1]}:e===`/`||e===``?{type:`home`}:{type:`404`}}async function w(){let e=C();e.type===`product`?(S=new p(e.productId),await S.render()):e.type===`home`?(S=null,await T()):document.querySelector(`#root`).innerHTML=`
      <main class="max-w-md mx-auto px-4 py-4">
        <div class="text-center my-4 py-20 shadow-md p-6 bg-white rounded-lg">
          ...
          <a href="${g(`/`)}" class="inline-block px-6 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 transition-colors">홈으로</a>
        </div>
      </main>
    `}async function T(){let e=y();v={...v,...e},v.loading=!0,E();let t=await c({page:1,limit:v.limit,search:v.search,sort:v.sort,category1:v.category1,category2:v.category2}),n=await u();v.products=t.products,v.categories=n,v.total=t.pagination.total,v.page=1,v.hasMore=t.products.length===v.limit,v.loading=!1,E(),k(),f.init()}function E(){document.body.querySelector(`#root`).innerHTML=s(v)}async function D(){if(!(v.loading||!v.hasMore)){v.loading=!0,E();try{let e=await c({page:v.page+1,limit:v.limit,sort:v.sort,search:v.search,category1:v.category1,category2:v.category2});v.products=[...v.products,...e.products],v.page+=1,v.hasMore=e.products.length===v.limit,v.loading=!1,E()}catch(e){console.error(`추가 상품 로드 실패:`,e),v.loading=!1,E()}}}function O(){window.innerHeight+window.scrollY>=document.body.offsetHeight-100&&D()}function k(){x||(document.body.querySelector(`#root`).addEventListener(`keydown`,async e=>{if(e.target.id===`search-input`&&e.key===`Enter`){let t=e.target.value.trim();v.search=t,v.page=1,v.loading=!0,b(),E();let n=await c({search:t,page:1,limit:v.limit,sort:v.sort,category1:v.category1,category2:v.category2});v.products=n.products,v.total=n.pagination.total,v.hasMore=n.products.length===v.limit,v.loading=!1,E()}}),document.body.querySelector(`#root`).addEventListener(`click`,async e=>{if(e.target.matches(`.product-image img`)||e.target.closest(`.product-image`)){let t=e.target.closest(`.product-card`);if(t){let e=t.dataset.productId;if(e){window.history.pushState({},``,g(`/product/${e}`)),await w();return}}}if(e.target.closest(`#cart-icon-btn`)){f.openModal();return}if(e.target.classList.contains(`add-to-cart-btn`)){let t=e.target.dataset.productId,n=v.products.find(e=>e.productId.toString()===t);if(n){let e={id:n.productId,name:n.title,price:parseInt(n.lprice),image:n.image,brand:n.brand||``};f.addItem(e)}return}if(e.target.dataset.category1){let t=e.target.dataset.category1;v.category1=t,v.category2=``,v.page=1,v.loading=!0,b(),E();let n=await c({category1:t,page:1,limit:v.limit,sort:v.sort,search:v.search});v.products=n.products,v.total=n.pagination.total,v.hasMore=n.products.length===v.limit,v.loading=!1,E()}if(e.target.dataset.category2){let t=e.target.dataset.category2;v.category2=t,v.page=1,v.loading=!0,b(),E();let n=await c({category1:v.category1,category2:t,page:1,limit:v.limit,sort:v.sort,search:v.search});v.products=n.products,v.total=n.pagination.total,v.hasMore=n.products.length===v.limit,v.loading=!1,E()}if(e.target.dataset.breadcrumb===`reset`){v.category1=``,v.category2=``,v.page=1,v.loading=!0,b(),E();let e=await c({page:1,limit:v.limit,sort:v.sort,search:v.search});v.products=e.products,v.total=e.pagination.total,v.hasMore=e.products.length===v.limit,v.loading=!1,E()}if(e.target.dataset.breadcrumb===`category1`){v.category2=``,v.page=1,v.loading=!0,b(),E();let e=await c({category1:v.category1,page:1,limit:v.limit,sort:v.sort,search:v.search});v.products=e.products,v.total=e.pagination.total,v.hasMore=e.products.length===v.limit,v.loading=!1,E()}}),document.body.querySelector(`#root`).addEventListener(`change`,async e=>{if(e.target.id===`limit-select`){let t=parseInt(e.target.value);v.limit=t,v.page=1,v.loading=!0,b(),E();let n=await c({limit:t,page:1,search:v.search,sort:v.sort,category1:v.category1,category2:v.category2});v.products=n.products,v.total=n.pagination.total,v.hasMore=n.products.length===t,v.loading=!1,E()}if(e.target.id===`sort-select`){let t=e.target.value;v.sort=t,v.page=1,v.loading=!0,b(),E();let n=await c({sort:t,page:1,search:v.search,limit:v.limit,category1:v.category1,category2:v.category2});v.products=n.products,v.total=n.pagination.total,v.hasMore=n.products.length===v.limit,v.loading=!1,E()}}),window.addEventListener(`scroll`,O),x=!0)}async function A(){window.addEventListener(`popstate`,w),await w()}_().then(A);